...
// 闭包顿悟 内部函数引用的的外部函数的变量a，它会把a存在外部函数的内存中，且外部函数销毁时，不会消失。所以循环点击按钮点击的i不是最后循环的i，而是每次循环后，存在内存中的i。为什么加自执行函数就可以，是因为for没循环一次，外部函数(自执行函数)就会执行一次（内部函数就会再定义一次，而每定义一次内部函数就会产生闭包，调用内部函数不会产生新的闭包，是定义了就有），会产生多个闭包对象数据，闭包赋值后没有消失，没有置为null。var没有块定义域，所以点击的按钮不在块里找i，找出去函数的全局的，let可以存在块里，存住了。
